 ...
 */
 ...
 #define ACCEL_STEP 1    // Size of acceleration steps
 ...
 // Move both motors simultaneously with left motor boost and gradual acceleration
 void move_motors(int left_target, int right_target) {
     ...
     // Determine number of increments needed for the smoothest transition
     ...
     for (int i = 0; i < num_steps; i++) {
         ...
         display_motor_values((int)left_current, (int)right_current);
 ...
 // Move robot forward with left motor boost and gradual acceleration
 void forward() {
     lcd_cursor(0, 0);
 ...
 // Move robot backward with gradual acceleration
 void backward() {
     lcd_cursor(0, 0);
 ...
 // Stop both motors with gradual deceleration
 void stop() {
     lcd_cursor(0, 0);
 ...
 int main(int argc, char *argv[]) {
     ...
     clear_screen();
     print_string("Gradual Accel Test");
     _delay_ms(2000);
     
     while (1) {
         // Test forward motion with boost and gradual acceleration
         forward();
         _delay_ms(3000);  // Run forward for a while
         
         // Test stopping with gradual deceleration
         stop();
         _delay_ms(2000);
         
         // Test backward motion with gradual acceleration
         backward();
         _delay_ms(3000);
         
         // Stop with gradual deceleration
         stop();
         _delay_ms(2000);
     }
